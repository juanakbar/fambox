...
<div id="edit-product-{{ $item->id }}"
    ...
    <h5 id="drawer-label"
        ...
        Product</h5>
    <button type="button" data-drawer-dismiss="edit-product-{{ $item->id }}" aria-controls="create-product"
    ...
    <form action="{{ route('products.update', $item->slug) }}" method="post" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="name" id="name"
                    ...
                    placeholder="Type product name" value="{{ old('name', $item->name) }}">
            </div>
            <div>
                <label for="price" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Price</label>
                <input type="number" name="price" id="price"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                    placeholder="100000" value="{{ old('price', $item->price) }}">
            </div>
            <div>
                ...
                <select id="category_id" name="category_id"
                    ...
                    <option selected="{{ $item->category->id }}" value="{{ $item->category->id }}">
            ...
            <div>
                ...
                <select id="satuan" name="satuan"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                    <option selected="{{ $item->satuan }}" value="{{ $item->satuan }}">{{ $item->satuan }}</option>
            ...
            <div class="bottom-0 left-0 flex justify-center w-full pb-4 space-x-4 md:px-4 md:absolute">
                ...
                <button type="button" data-drawer-dismiss="edit-product-{{ $item->id }}"
